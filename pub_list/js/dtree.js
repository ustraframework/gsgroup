...
dTree.prototype.node = function(node, nodeId) {
    ...
    if (node.url) {
        str += '<span class="word_wrap"><a id="s' + this.obj + nodeId + '" class="' + ((this.config.useSelection) ? ((node._is ? 'nodeSel' : 'node')) : 'node') + '" href="' + (node.url||'javascript:;') + '"';
    ...
    if (node.url && !pop && dTree.options.target !== '_blank') {
        str += ' <a href="' + node.url + '" target="_blank"><img src="'+this.icon.popup+'"></a></span>';
    }
